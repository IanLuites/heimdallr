defmodule Heimdallr.Application do
  ...
  def start(_type, _args) do
    if match?({_, 0}, System.cmd("which", ["inotifywait"])) do
      patch_credo()

      children = [
        Heimdallr.Indexer,
        Heimdallr.Watcher.child_spec(~W(./lib ./test))
      ]

      opts = [strategy: :one_for_one, name: Heimdallr.Supervisor]
      Supervisor.start_link(children, opts)
    else
      Logger.warn("Heimdallr: inotifywait unavailable, file watcher disabled.")
      {:ok, self()}
    end
  end
